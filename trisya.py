...
def setting():
	...
	print(f'{P}1.  Old{M} (Not Recommend\n{P}2.  New{H} (Recommend)\n{P}3.  Random{K} (Very Recommend')
	hu = input('Pilih : ')
	if hu in ['1','01']:
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'𝙩𝙧𝙞𝙨𝙮𝙖[deep_white]{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
